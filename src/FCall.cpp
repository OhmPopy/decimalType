...
typedef enum
{
	DECIMAL_OK,
	DECIMAL_OVERFLOW,
	DECIMAL_INVALID_ARGUMENT,
	DECIMAL_DIVBYZERO,
	DECIMAL_ARGUMENT_OUT_OF_RANGE
} DecimalStatus;

typedef enum DecimalCompareResult
{
	DECIMAL_CMP_LT = -1,
	DECIMAL_CMP_EQ,
	DECIMAL_CMP_GT
...
#define DOUBLE_BIAS 1022

// Single floating point Bias
#define SINGLE_BIAS 126
...
#if BYTE_ORDER == BIG_ENDIAN
const Il2CppDouble_double ds2to64 = {{0, DOUBLE_BIAS + 65, 0, 0}};
#else
const Il2CppDouble_double ds2to64 = {{0, 0, DOUBLE_BIAS + 65, 0}};
#endif
...
// Returns: DECIMAL_OK, or DECIMAL_INVALID_ARGUMENT
static DecimalStatus
decimal_to_double_result(Il2CppDecimal* input, double* result)
{
	SPLIT64 tmp;
	double dbl;

	if (DECIMAL_SCALE(*input) > DECMAX || (DECIMAL_SIGN(*input) & ~DECIMAL_NEG) != 0)
		return DECIMAL_INVALID_ARGUMENT;
...
// Returns: DECIMAL_OK, or DECIMAL_INVALID_ARGUMENT
static DecimalStatus
decimal_to_float_result(Il2CppDecimal* input, float* result)
{
	double dbl;
	...
	//
	decimal_to_double_result(input, &dbl);
	*result = (float)dbl;
	return DECIMAL_OK;
}

static DecimalStatus
DecAddSub(System::Il2CppDecimal* left, Il2CppDecimal* right, Il2CppDecimal* result, int8_t sign)
{
	uint32_t num[6];
	uint32_t pwr;
	int scale;
...
// Returns DECIMAL_OK or DECIMAL_OVERFLOW
static DecimalStatus
decimal_from_double(double input_d, Il2CppDecimal* result)
{
	...
	//
	if ((exp = input.s.exp - DOUBLE_BIAS) < -94)
	{
...
// Decimal multiply
// Returns: DECIMAL_OVERFLOW or DECIMAL_OK
static DecimalStatus decimal_multiply_result(Il2CppDecimal* left, Il2CppDecimal* right, Il2CppDecimal* result)
{
	SPLIT64 tmp;
	SPLIT64 tmp2;
	SPLIT64 tmp3;
...
//
// Returns: DECIMAL_INVALID_ARGUMENT, DECIMAL_OK
//
static DecimalStatus
decimal_round_result(Il2CppDecimal* input, int cDecimals, Il2CppDecimal* result)
{
	uint32_t num[3];
	uint32_t rem;
	uint32_t sticky;
...
// Returns DECIMAL_OK or DECIMAL_OVERFLOW
static DecimalStatus
decimal_from_float(float input_f, Il2CppDecimal* result)
{
	int exp; // number of bits to left of binary point
	int power;
	uint32_t mant;
	...
	// barely reach 0.5, but smaller exponents will always round to zero.
	//
	if ((exp = input.s.exp - SINGLE_BIAS) < -94)
	{
		DECIMAL_SETZERO(*result);
		return DECIMAL_OK;
	}
...
// decimal_round_to_int - Decimal Int (round down to integer)
static void
decimal_round_to_int(Il2CppDecimal* pdecOprd, Il2CppDecimal* result)
{
...
static void
decimal_fix(Il2CppDecimal* pdecOprd, Il2CppDecimal* result)
{
...
namespace System
{
	double FCall::FCallToDouble(Il2CppDecimal d)
	{
		double result = 0.0;
		// Note: this can fail if the input is an invalid decimal, but for compatibility we should return 0
		decimal_to_double_result(&d, &result);
		return result;
	}

	int32_t FCall::FCallCompare(Il2CppDecimal* left, Il2CppDecimal* right)
	{
		uint32_t left_sign;
		uint32_t right_sign;
	...
	int32_t FCall::GetHashCode(Il2CppDecimal* _this)
	{
		double dbl;

		if (decimal_to_double_result(_this, &dbl) != DECIMAL_OK)
			return 0;
	...
	float FCall::ToSingle(Il2CppDecimal d)
	{
		float result = 0.0f;
		// Note: this can fail if the input is an invalid decimal, but for compatibility we should return 0
		decimal_to_float_result(&d, &result);
		return result;
	}

	void FCall::ConstructorDouble(Il2CppDecimal* _this, double value)
	{
		if (decimal_from_double(value, _this) == DECIMAL_OVERFLOW)
		{
			RaiseOverflowException();
			return;
		}
	...
	void FCall::ConstructorFloat(Il2CppDecimal* _this, float value)
	{
		if (decimal_from_float(value, _this) == DECIMAL_OVERFLOW)
		{
			RaiseOverflowException();
			return;
		}
	...
	void FCall::FCallAddSub(Il2CppDecimal* left, Il2CppDecimal* right, uint8_t sign)
	{
	...
	void FCall::FCallDivide(Il2CppDecimal* left, Il2CppDecimal* right)
	{
	...
	void FCall::FCallFloor(Il2CppDecimal* d)
	{
		Il2CppDecimal decRes;

		decimal_round_to_int(d, &decRes);
	...
	void FCall::FCallMultiply(Il2CppDecimal* d1, Il2CppDecimal* d2)
	{
		Il2CppDecimal decRes;

		DecimalStatus status = decimal_multiply_result(d1, d2, &decRes);
		if (status != DECIMAL_OK)
		{
			RaiseOverflowException();
			return;
		}
	...
	void FCall::FCallRound(Il2CppDecimal* d, int32_t decimals)
	{
		Il2CppDecimal decRes;
		...
		{
			RaiseArgumentOutOfRangeException("d");
			return;
		}

		decimal_round_result(d, decimals, &decRes);
	...
	void FCall::FCallTruncate(Il2CppDecimal* d)
	{
		Il2CppDecimal decRes;

		decimal_fix(d, &decRes);
	...
	double FCall::FCallToDouble(Decimal d)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = d.hi;
		decimal_left.v.v.Mid32 = d.mid;
		decimal_left.v.v.Lo32 = d.lo;
		decimal_left.u.signscale = d.flags >> 16;

		return FCallToDouble(decimal_left);
	}

	int32_t FCall::FCallCompare(Decimal* left, Decimal* right)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = left->hi;
		decimal_left.v.v.Mid32 = left->mid;
		decimal_left.v.v.Lo32 = left->lo;
		decimal_left.u.signscale = left->flags >> 16;
		Il2CppDecimal decimal_right;
		decimal_right.Hi32 = right->hi;
		decimal_right.v.v.Mid32 = right->mid;
		decimal_right.v.v.Lo32 = right->lo;
		decimal_right.u.signscale = right->flags >> 16;

		return FCallCompare(&decimal_left, &decimal_right);
	}

	int32_t FCall::FCallToInt32(Decimal* d)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = d->hi;
		decimal_left.v.v.Mid32 = d->mid;
		...
		return FCallToInt32(decimal_left);
	}

	int32_t FCall::GetHashCode(Decimal* _this)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = _this->hi;
		decimal_left.v.v.Mid32 = _this->mid;
		decimal_left.v.v.Lo32 = _this->lo;
		decimal_left.u.signscale = _this->flags >> 16;
		return GetHashCode(&decimal_left);;
	}

	float FCall::ToSingle(Decimal d)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = d.hi;
		decimal_left.v.v.Mid32 = d.mid;
		...
		return ToSingle(decimal_left);
	}

	void FCall::ConstructorDouble(Decimal* _this, double value)
	{
		Il2CppDecimal decimal;
		ConstructorDouble(&decimal, value);

		_this->hi = decimal.Hi32;
		_this->mid = decimal.v.v.Mid32;
		_this->lo = decimal.v.v.Lo32;
		_this->flags = decimal.u.signscale << 16;
	}

	void FCall::ConstructorFloat(Decimal* _this, float value)
	{
		Il2CppDecimal decimal;
		ConstructorFloat(&decimal, value);
	...
	void FCall::FCallAddSub(Decimal* left, Decimal* right, uint8_t sign)
	{
		Il2CppDecimal decimal_left;
		//memcpy(&decimal_left, left, sizeof(Il2CppDecimal)); //bdki slow
		...
		decimal_left.u.signscale = left->flags >> 16;
		Il2CppDecimal decimal_right;
		//memcpy(&decimal_right, right, sizeof(Il2CppDecimal));
		...
		FCallAddSub(&decimal_left, &decimal_right, sign);

		left->hi = decimal_left.Hi32;
		left->mid = decimal_left.v.v.Mid32;
		left->lo = decimal_left.v.v.Lo32;
		left->flags = decimal_left.u.signscale << 16;
	}

	void FCall::FCallDivide(Decimal* left, Decimal* right)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = left->hi;
		decimal_left.v.v.Mid32 = left->mid;
		...
		FCallDivide(&decimal_left, &decimal_right);
	...
	void FCall::FCallFloor(Decimal* d)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = d->hi;
		decimal_left.v.v.Mid32 = d->mid;
		...
		FCallFloor(&decimal_left);

		d->hi = decimal_left.Hi32;
		d->mid = decimal_left.v.v.Mid32;
		d->lo = decimal_left.v.v.Lo32;
		d->flags = decimal_left.u.signscale << 16;
	}

	void FCall::FCallMultiply(Decimal* left, Decimal* right)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = left->hi;
		decimal_left.v.v.Mid32 = left->mid;
		...
		FCallMultiply(&decimal_left, &decimal_right);
	...
	void FCall::FCallRound(Decimal* d, int32_t decimals)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = d->hi;
		decimal_left.v.v.Mid32 = d->mid;
		...
		FCallRound(&decimal_left, decimals);
	...
	void FCall::FCallTruncate(Decimal* d)
	{
		Il2CppDecimal decimal_left;
		decimal_left.Hi32 = d->hi;
		decimal_left.v.v.Mid32 = d->mid;
	...
	void FCall::RaiseOverflowException()
	{
		throw System::OverflowException();
	}

	void FCall::RaiseDivideByZeroException()
	{
		throw DivideByZeroException();
	}

	void FCall::RaiseArgumentOutOfRangeException(std::string c)
	{
		throw ArgumentOutOfRangeException(c);
	}
}
